...
def get_pro_data(ticker, oi_info):
    try:
        ...
        # ── FIX 3 (v2): Vol Ratio — cumulative today volume vs fair prev-day comparison ──
        # Purana bug: sirf aakhri candle ka volume leta tha, jo subah jaldi (9:15-9:20)
        # hamesha chhota dikhta tha kyunki poora din ka volume abhi jama hi nahi hua hota.
        # Naya fix: aaj subah se ab tak ka TOTAL volume lo, aur usse pichle din ke
        # UTNE HI candles ke expected (fair) volume se compare karo.
        prev_avg_vol_per_candle = float(prev_data['Volume'].mean())
        num_candles_today = len(today_data)

        if num_candles_today < 2 or prev_avg_vol_per_candle <= 0:
            # Bahut kam candles bani hain abhi (jaise subah 9:15-9:20) —
            # reliable ratio nahi bana sakte, isliye None (N/A) dikhao,
            # jhoothi 0.0x mat dikhao
            vol_ratio = None
        else:
            curr_vol_cumulative = float(today_data['Volume'].sum())
            expected_vol_by_now = prev_avg_vol_per_candle * num_candles_today
            vol_ratio = round(curr_vol_cumulative / expected_vol_by_now, 1) if expected_vol_by_now > 0 else None

        score = 0                        # base 0 karo
        if cp > vwap:       score += 34
        if ema9 > ema21:    score += 33
        if vol_ratio is not None and vol_ratio > 1.2: score += 33
        ...
        return {
            ...
            "VOL RATIO":  f"{vol_ratio}x" if vol_ratio is not None else "N/A",
            "STRENGTH":   f"{score}%",
            "STOP LOSS":  sl,
            "TARGET":     tgt,
        }
